...
- Path: `packages/apps/logger/src/contracts`
...
### `action-contracts.ts`
- Purpose: Declares action contracts for structured logging and lifecycle event instrumentation APIs.
- Input shape: Logger scope identifiers, lifecycle state records, error payloads, and event metadata fields.
- Output shape: Typed `ActionContract[]` for lifecycle logger integration documentation and checks.
- Role interaction rules:
...
- Logic gates summary:
  - Action names must align with logger runtime helpers.
  - Output schema must preserve lifecycle identity and sanitized metadata.
  - Source paths must map to logger runtime modules.
- Functions (all):
  - `action`
- Exports:
  - `ACTION_CONTRACTS`
...
- Key snippets and use-case mapping:
  - `L1-L22`: Defines role gates and contract helper for logger action rows.
  - `L23-L57`: Declares logger action inventory and typed output contracts.
