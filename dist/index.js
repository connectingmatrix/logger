...
class ProcessMonitoringRuntime {
    ...
    health() { return { name: '@connectingmatrix/logger/process-monitor', status: 'ok', checkedAt: nowIso(), details: { processes: this.rows.size, logs: this.globalLogs.length, socketsBound: Boolean(this.sockets) } }; }
}
class LoggerRuntime {
    constructor() {
...
export const createPackage = () => ({
    ...
    routes: [
        ...
        { method: 'GET', path: '/logger/launcher', handler: (request) => createPackageStatusPanel(request.context ?? {}) },
    ],
});
